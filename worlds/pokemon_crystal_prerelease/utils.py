...
def __adjust_options_johto_only(world: "PokemonCrystalWorld"):
    if world.options.johto_only:
        ...
        if world.options.goal == Goal.option_true_scholar and world.options.johto_only != JohtoOnly.option_off:
            world.options.goal.value = Goal.option_elite_four
            logging.warning(
                "Pokemon Crystal: True Scholar goal is incompatible with Johto Only. "
                "Changing goal to Elite Four for player %s.",
                world.player_name)
...
def convert_to_ingame_text(text: str, string_terminator: bool = False) -> list[int]:
    charmap = {
        ...
        "9": 0xff, "_": 0xe3, "♀": 0xf5, "$": 0xf0, "£": 0xf0, "€": 0xf0
    }
    apostrophe_specials = {
